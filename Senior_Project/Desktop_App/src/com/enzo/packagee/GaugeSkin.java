...
public class GaugeSkin extends SkinBase<Gauge> implements Skin<Gauge> {
    ...
    private Pane                     pane;							//this i believe is the layout type
    private Region                   background;
    private Canvas                   ticksAndSectionsCanvas;
    private GraphicsContext          ticksAndSections;
    private Region                   threshold;						//this var is for threshold
    ...
    public GaugeSkin(Gauge gauge) {
        ...
        System.out.println("[GaugeSkin Class] Constructor() is Called");
    ...
    private void initGraphics() {
    	...
    	System.out.println("[GaugeSkin Class] initGraphics() is Called");
    	
        //Font.loadFont(getClass().getResourceAsStream("/eu/hansolo/enzo/fonts/opensans-semibold.ttf"), (0.06 * PREFERRED_HEIGHT)); // "OpenSans"

    	Font.loadFont(getClass().getResourceAsStream("opensans-semibold.ttf"), (0.06 * PREFERRED_HEIGHT)); // "OpenSans"
        ...
        System.out.println("[GaugeSkin Class] initGraphics() Region Obj is intialized");
        threshold = new Region();
        System.out.println("[GaugeSkin Class] initGraphics() threshold obj is linked with css");
        threshold.getStyleClass().setAll("threshold");
        
        //???????????
        System.out.println("[GaugeSkin Class] Rotate Obj is intialized");
        thresholdRotate = new Rotate(180 - getSkinnable().getStartAngle() - getSkinnable().getMinValue() * angleStep);
        System.out.println("[GaugeSkin Class] initGraphics() thresholdRotate:	"+thresholdRotate);
        System.out.println("[GaugeSkin Class] initGraphics() getSkinnable().getStartAngle():	"+getSkinnable().getStartAngle() );
        System.out.println("[GaugeSkin Class] initGraphics() getSkinnable().getMinValue():	"+getSkinnable().getMinValue() );
        System.out.println("[GaugeSkin Class] initGraphics() angleStep:	"+angleStep);
        ...
        //these are few of the nodes which i believe some how are added to the scene or stage in DemoGauge
        pane.getChildren().setAll(background,
                                  ...
                                  unit,
                                  value);
        System.out.println("[GaugeSkin Class] initGraphics() Guages nodes: "+ pane.getChildren().toArray().toString());
    ...
    private void handleMouseEvent(final MouseEvent MOUSE_EVENT) {
    	System.out.println("[GaugeSkin Class] handleMouseEvent() is called ");
        ...
        if (getSkinnable().isInteractive() && SRC.equals(threshold)) {
        	//System.out.println("MouseEvent Source:	"+ SRC.toString());
        	//System.out.println("MouseEvent Type:	"+ TYPE.toString());
        	
            if (MouseEvent.MOUSE_PRESSED == TYPE) {
            	//System.out.println("[GaugeSkin Class] "+TYPE.toString()+" On "+SRC.toString()+ " Occured");
            	//System.out.println("		Threshold Value:	"+ getSkinnable().getThreshold());
    ...
    private void resize() {
    	...
    	System.out.println("[GaugeSkin Class] resize() is called");
...
/*WHATEVER NOTES
 * 	this calss is related to gauage.css as most of the sectors are being used in the initGraphics() method 
 * 
 * 	RESEARCH
 * 	what is region in javafx
 * */
